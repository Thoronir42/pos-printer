/// This file is ejected from escpos-usb to fix usb event handler usage
///   since usb@2.0.0, usb events are under usb.usb namespace.
...
USB.prototype.open = function (callback){
  ...
  const printerIfaces = this.device.interfaces.filter(function(iface) {
    if (!iface.descriptor || iface.descriptor.bInterfaceClass !== IFACE_CLASS.PRINTER) {
      return false;
    }

    return iface.endpoints.some(function(endpoint) {
      return endpoint.direction == 'out';
    });
  });

  const candidateIfaces = printerIfaces.length ? printerIfaces : this.device.interfaces;
  let pending = candidateIfaces.length;
  let lastError = null;

  if (!pending) {
    callback && callback(new Error('Can not find interface from printer'));
    return this;
  }

  candidateIfaces.forEach(function(iface){
    (function(iface){
      iface.setAltSetting(iface.altSetting, function(){
        ...
        try {
          ...
          } else if(--pending === 0 && !self.endpoint){
            callback && callback(lastError || new Error('Can not find endpoint from printer'));
          }
        } catch (e) {
